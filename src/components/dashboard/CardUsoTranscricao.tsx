...
interface CardUsoTranscricaoProps {
  uso: UsoTranscricao;
}

function CardUsoTranscricao({ uso }: CardUsoTranscricaoProps) {
  const semTranscricao = uso.limiteSegundos === 0;
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm font-semibold text-slate-900">
              Transcrição não disponível no seu plano
            </p>
            <p className="mt-0.5 text-xs text-slate-500">
              Faça upgrade para gravar consultas e usar a IA na anamnese.
            </p>
            <Link
              href="/configuracoes"
      ...
      <div className="mt-4 space-y-2">
        <div className="h-2 overflow-hidden rounded-full bg-slate-100">
          <div
            ...
            style={{ width: `${Math.max(2, Math.round(uso.percentual))}%` }}
          />
        </div>
        <div className="flex items-baseline justify-between gap-2 text-xs">
          <p className="text-slate-700">
            ...
            de {uso.minutosLimite} min usados
          </p>
...
export default CardUsoTranscricao;
